...
BASE_URL = "https://www.geocat.ch/geonetwork/srv/eng/catalog.search"

REDIS_HOST = "localhost"
REDIS_PORT = 6379
REDIS_QUEUE = "neo4j_writes_ch"
...
class SiteGraphCrawlerNeo4j:
    def __init__(self, max_depth=10, external_max_hops=2, run_id="default", concurrency=10):
        ...
        self.visited = set()  # Full URIs visited (for crawl loop prevention)
    ...
    def _lpush(self, msg: str):
        ...
        for attempt in range(1, REDIS_MAX_RETRIES + 1):
            try:
                ...
                print(f"[crawler/ch] Redis connection lost (attempt {attempt}/{REDIS_MAX_RETRIES}): {e}")
                time.sleep(REDIS_RETRY_DELAY)
                try:
                    self.redis = redis.Redis(host=REDIS_HOST, port=REDIS_PORT)
                    self.redis.ping()
                    print("[crawler/ch] Redis reconnected.")
                except Exception:
                    pass
        print(f"[crawler/ch] ERROR: could not push to Redis after {REDIS_MAX_RETRIES} attempts — message dropped.")
    ...
    async def crawl_page(self, page, url, depth=0, external_hops=0):
        ...
        print(f"[Depth {depth}] Crawling: {url[:80]}... (site={current_site})")
        ...
        try:
            ...
            for link_url in links_to_follow[:10]:
                new_external_hops = (
    ...
    async def crawl_from_seeds(self, seed_urls):
        """Start crawling from a list of seed URLs concurrently"""
...
def load_metadata_links(filepath='ch_metadata_links.txt'):
...
async def main():
    ...
    parser.add_argument(
        ...
        help="Identifier for this crawl run (e.g. 'sites-ch-2026-02-23')",
    )
    parser.add_argument(
    ...
    seed_links = load_metadata_links(
        os.path.join(SCRIPT_DIR, '..', 'ch_metadata_links.txt')
    )
