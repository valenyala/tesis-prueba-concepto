...
class WebGraphCrawler:
    ...
    def get_page_label(self, url):
        ...
        parsed = urlparse(url)
        # Use the last path segment(s) to avoid cutting words
    ...
    async def crawl_page(self, page, url, depth=0, external_hops=0):
        ...
        try:
            ...
            # Use the actual browser URL (without normalization) for visited tracking
            actual_url = page.url
    ...
    def visualize_graph(self, output_file='web_structure.png', show=True):
        ...
        plt.title('Web Structure Graph - Spain IDEE GeoNetwork', fontsize=16)
        plt.axis('off')
        plt.tight_layout()
...
async def main():
    ...
    # Visualize the graph
    crawler.visualize_graph('web_structure.png', show=False)
